...
async function main() {
  ...
  for (const file of process.argv.slice(2)) {
    ...
    assert.equal(updated.cache_version, encoder.HLS_CACHE_VERSION)
    const directory = path.join(process.env.CACHE_DIR, 'channels', channel.slug, 'videos', hash, `v${encoder.HLS_CACHE_VERSION}`)
